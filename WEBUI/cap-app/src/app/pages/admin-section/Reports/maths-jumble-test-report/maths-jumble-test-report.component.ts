...
@Component({
  selector: 'app-maths-jumble-test-report',
  templateUrl: './maths-jumble-test-report.component.html',
  styleUrls: ['./maths-jumble-test-report.component.scss']
})
export class MathsJumbleTestReportComponent implements OnInit {
  ...
  loadAdminExamQuestions(event: LazyLoadEvent) {
    ...
    this.adminCommonEndpointsService.getAdminSATTesJumbletReport(this.user.UserId, this.user.Usertype).subscribe(result => {
  ...
  viewQuestionsByExamId(examId) {
    this.spinnerService.show();
    this.adminCommonEndpointsService.getAllQuestionByExamId(examId.ExamId, studentTestEnum.JumbleTest).toPromise().then(result => {
