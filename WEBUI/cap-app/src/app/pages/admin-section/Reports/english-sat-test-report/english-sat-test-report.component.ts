...
@Component({
  selector: 'app-english-sat-test-report',
  templateUrl: './english-sat-test-report.component.html',
  styleUrls: ['./english-sat-test-report.component.scss']
})
export class EnglishSatTestReportComponent implements OnInit {
  ...
  loadAdminExamQuestions(event: LazyLoadEvent) {
    ...
    this.adminCommonEndpointsService.getAdminPracticeExamSATEnglishReport(this.user.UserId, this.user.Usertype, studentTestEnum.SatTest).subscribe(result => {
  ...
  viewQuestionsByExamId(examId) {
    this.spinnerService.show();
    this.adminCommonEndpointsService.getAllEnglishQuestionbyexamid(examId.ExamId, studentTestEnum.SatTest).toPromise().then(result => {
