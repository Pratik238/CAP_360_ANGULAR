...
export class SATTestReportComponent implements OnInit {
  ...
  loadAdminExamQuestions(event: LazyLoadEvent) {
    ...
    this.adminCommonEndpointsService.getAdminSATTestReport(this.user.UserId, this.user.Usertype).subscribe(result => {
  ...
  viewQuestionsByExamId(examId) {
    this.spinnerService.show();
    this.adminCommonEndpointsService.getAllQuestionByExamId(examId.ExamId, studentTestEnum.SatTest).toPromise().then(result => {
