...
@Component({
  selector: 'app-english-practicetest-report',
  templateUrl: './english-practicetest-report.component.html',
  styleUrls: ['./english-practicetest-report.component.scss']
})
export class EnglishPracticetestReportComponent implements OnInit {
  ...
  loadAdminExamQuestions(event: LazyLoadEvent) {
    ...
    this.adminCommonEndpointsService.getAdminPracticeExamSATEnglishReport(this.user.UserId, this.user.Usertype, studentTestEnum.PracticeTest).subscribe(result => {
  ...
  viewQuestionsByExamId(examId) {
    this.adminCommonEndpointsService.getAllEnglishQuestionbyexamid(examId.ExamId, studentTestEnum.PracticeTest).toPromise().then(result => {
